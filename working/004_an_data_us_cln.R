...
p<-p+scale_color_manual(values=c("red", "dark red", "grey", "blue", "black", "light blue"))
p1<-p+labs(title="Infection Rate per 100,000", subtitle="United States by State Code", x="Date", y="Infections Per 100,000")

p<-ggplot(covid_st_d[state_cln %in% c("ca", "ny", "il", "az", "fl", "tx")], aes(x=src, y=infections_pp100, color=state_cln))+geom_line()+facet_wrap(~state_cln, ncol=3, nrow=3)
p<-p+theme_classic()+theme(legend.position = "none")
p<-p+scale_color_manual(values=c("red", "dark red", "grey", "blue", "black", "light blue"))
p2<-p+labs(title="Infection Rate per 100,000", subtitle="United States by State Code", x="Date", y="Infections Per 100,000")

p<-ggplot(covid_st_d[state_cln %in% c("ca", "ny", "il", "az", "fl", "tx") & src>=as.Date("2020-03-15")], aes(x=src, y=SMA_pct_chg, color=state_cln))+geom_line()+facet_wrap(~state_cln, ncol=3, nrow=3)
p<-p+theme_classic()+theme(legend.position = "none")
p<-p+scale_color_manual(values=c("red", "dark red", "grey", "blue", "black", "light blue"))
...
p<-ggplot(covid_st_d[state_cln %in% c("ca", "ny", "il", "az", "fl", "tx")], aes(x=src, y=log(infections_pp100), color=state_cln))+geom_line()+facet_wrap(~state_cln, ncol=3, nrow=3)
p<-p+theme_classic()+theme(legend.position = "none")
p<-p+scale_color_manual(values=c("red", "dark red", "grey", "blue", "black", "light blue"))
p4<-p+labs(title="Infections Rate per 100,000", subtitle="United States by State Code", x="Date", y="LN(Infections Per 100,000)")

p<-ggplot(covid_st_d[state_cln %in% c("ca", "ny", "il", "az", "fl", "tx")], aes(x=src, y=log(infections_pp100),group=state_cln, color=state_cln))+geom_line()
...
p<-ggplot(covid_st_d[state_cln %in% c("ca", "ny", "il", "az", "fl", "tx") & src>=as.Date("2020-03-15")], aes(x=src, y=SMA_pct_chg,group=state_cln, color=`Stay-at-Home`))+geom_line()
p<-p+theme_classic()+theme(legend.position = "bottom")
p<-p+scale_color_manual(values=c("red", "grey"))
